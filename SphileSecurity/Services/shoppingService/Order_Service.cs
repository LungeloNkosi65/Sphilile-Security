...
namespace SphileSecurity.Services.shoppingService
{
        ...
        public void AddOrderItems(Order order, List<Cart_Item> items)
        {
            foreach (var item in items)
            {
               var customer= order.Customer.Email;
                var discountPrice = 0.00M;
                try
                {
                    if (BILogic.GetDiscountRate(customer) != null)
                    {
                        discountPrice = BILogic.CalcDiscountPrice((decimal)item.price);
                    }
                    else
                    {
                        discountPrice = (decimal)item.price;
                    }
