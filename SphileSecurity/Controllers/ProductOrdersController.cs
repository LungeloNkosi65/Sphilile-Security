...
namespace ImfuyoRanch.Controllers
{
    public class ProductOrdersController : Controller
    {
        ...
        //private ProductOrderRepository _productOrdersRepository = new ProductOrderRepository();
        //private CartItemRepository _CartItemRepository  = new CartItemRepository();
        //private CustomerOrderRepository _CustomerOrderRepository  = new CustomerOrderRepository();
        CartBusiness cart_Service = new CartBusiness();

        // GET: FoodOrders
        public ActionResult Index()
        {
            var UserName = User.Identity.GetUserName();
            return View(db.ProductOrder.Where(x => x.UserEmail == UserName && x.OrderStatus != "Checked Out"));
        }
        ...
        public ActionResult COnfirmOrder()
        {
            ...
            var confirm = db.ProductOrder.ToList(); //db.productrders.ToList();
            var cart = db.ProductOrder.ToList(); //db.Cart_Items.ToList();
           ...
           // _CustomerOrderRepository.Insert(mealOrder);
            db.CustomerOrder.Add(mealOrder);
            db.SaveChanges();
            ...
            foreach (var item in confirm)
            {
                foreach (var i in cart)
                {
                    ...
                    {
                        var statusUpdate = db.ProductOrder.Where(l=>l.cart_item_id== item.cart_item_id).FirstOrDefault(); //db.productrders.Find(item.cart_item_id);
