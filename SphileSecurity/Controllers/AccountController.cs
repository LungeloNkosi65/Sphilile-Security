    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> RegisterEmployee(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await UserManager.CreateAsync(user, model.Password=passord);
                model.ConfirmPassword = passord;
                ...
                employee.DateRegisterd = DateTime.Now;
                if (result.Succeeded)
                {
                    db.Employees.Add(employee);
                    db.SaveChanges();
                    //await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
                    UserManager.AddToRole(user.Id, "Security");
